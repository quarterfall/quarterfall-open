...
export async function sendEmail(options: SendEmailOptions) {
    ...
    // rewrite email address if not production environment
    if (environment !== "production") {
        for (const personalization of options.personalizations) {
            for (const to of personalization.to) {
                to.email = `dev+${to.email.replace(
                    "@",
                    "_at_"
                )}@quarterfall.com`;
            }
            if (personalization.cc) {
                for (const cc of personalization.cc) {
                    cc.email = `dev+${cc.email.replace(
                        "@",
                        "_at_"
                    )}@quarterfall.com`;
                }
            ...
            if (personalization.bcc) {
                for (const bcc of personalization.bcc) {
                    bcc.email = `dev+${bcc.email.replace(
                        "@",
                        "_at_"
                    )}@quarterfall.com`;
                }
    ...
    // put dev@quarterfall.com in BCC - still needed because Microsoft doesn't support + addresses yet
    if (environment !== "production") {
        for (const personalization of options.personalizations) {
            personalization.bcc = personalization.bcc || [];
            personalization.bcc.push({
                email: "dev@quarterfall.com",
            });
        }
    }
    ...
    const mail: any = {
        ...
        from: options.from,
        headers: {
            "List-Unsubscribe": "<mailto:support@quarterfall.com>",
        },
    };
    if (options.reply_to) {
