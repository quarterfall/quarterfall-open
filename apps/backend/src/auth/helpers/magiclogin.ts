...
class MagicLoginStrategy {
    ...
    authenticate(
        ...
        if (
            ...
            ((req.query.token || req.body?.token) as string).match(
                patterns.email
            )
        ) {
            payload = {
    ...
    send = (req: Request, res: Response): void => {
        ...
        if (
            ...
            res.status(400).send(
                "Content-Type must be application/json when using POST method."
            );
            return;
        }

        if (!payload.emailAddress) {
            res.status(400).send("Please specify the emailAddress.");
            return;
        }
