...
export class CourseAnalyticsBlockDefaultsResolver {
    @Authorized()
    @Mutation((returns) => Organization)
    public async addCourseAnalyticsBlockDefault(
        ...
        const blocks = await this.courseAnalyticsBlockDefaults(organization);
        blocks.push(block);

        // arrange the blocks by type so that moving by index still works
        organization.courseAnalyticsBlockDefaults = [
        ...
        log.notice(
            `Course analytics block template with id ${block.id} added to organization "${organization.name}" (${organization.id}).`
        );
    ...
    public async updateCourseAnalyticsBlockDefault(
        @Ctx() context: RequestContext,
        ...
        log.notice(
            `Course analytics block default with id ${block.id} updated.`,
            input
        );
    ...
    @Authorized()
    @Mutation((returns) => Organization)
    public async deleteCourseAnalyticsBlockDefault(
        @Ctx() context: RequestContext,
        @Arg("id", (type) => ID) id: string
    ) {
        const { organization } = context;
    ...
    @Authorized()
    @Mutation((returns) => Organization)
    public async moveCourseAnalyticsBlockDefaultToIndex(
        @Ctx() context: RequestContext,
        ...
        const oldIndex = organization.courseAnalyticsBlockDefaults.findIndex(
            (b) => b.equals(block._id)
        );
        if (
            oldIndex < 0 ||
            oldIndex >= organization.courseAnalyticsBlockDefaults.length
        ) {
            throw new ApolloError(
        ...
        const blocksBefore = await this.courseAnalyticsBlockDefaults(
            organization
        );
        ...
        // move the block
        arrayMove({
            array: organization.courseAnalyticsBlockDefaults,
            oldIndex,
            newIndex,
            newArray: organization.courseAnalyticsBlockDefaults,
        });
    ...
    public async courseAnalyticsBlockDefaults(@Root() root: IOrganization) {
        const blocks = await DBAnalyticsBlock.find({
            _id: { $in: root.courseAnalyticsBlockDefaults || [] },
        });
        ...
        return (root.courseAnalyticsBlockDefaults || []).map(
