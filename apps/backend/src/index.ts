...
import { setupEvents } from "./event/bootstrap";
...
async function checkDatabaseAndEnvironmentMatch() {
    const dbUri = config.mongodb.connectionUri;

    if (environment === "staging" && dbUri?.includes("production")) {
        log.error(
            "The database connection string is set to the production database, but the environment is set to staging."
        );
        process.exit(1);
    }

    if (environment === "production" && dbUri?.includes("staging")) {
        log.error(
            "The database connection string is set to the staging database, but the environment is set to production."
        );
        process.exit(1);
    }
}
...
(async function bootstrap() {
    ...
    // check if the database connection string is set to the correct database
    await checkDatabaseAndEnvironmentMatch();
