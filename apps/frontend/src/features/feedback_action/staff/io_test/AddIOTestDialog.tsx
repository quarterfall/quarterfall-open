...
export interface AddIOTestDialogProps {
    block: Block;
    action: Action;
    open: boolean;
    onClose?: () => void;
...
export function AddIOTestDialog(props: AddIOTestDialogProps) {
    ...
    const newTestName = () => {
        const names = (action?.ioTests || []).map((t) => t.name);
        for (let i = 1; i < names?.length + 1; i++) {
            if (names.indexOf(`ioTest${i}`) === -1) {
                return `ioTest${i}`;
            }
        }
        return `ioTest${(action?.ioTests || []).length + 1}`;
    };
    ...
    const defaultValues = {
        ...
        input: "",
        output: "",
        comparisonCode: "return desiredOutput === computedOutput",
    };
    ...
    const onSubmit = async (input: IOTest) => {
        ...
        setWaiting(false);
        showSuccessToast(
            t("assignment:ioTestAddedNotification", {
                testName: input.name,
            })
        );
        if (onClose) {
    ...
    return (
        ...
        >
            <form onSubmit={handleSubmit(onSubmit)}>
                <DialogTitle>{t("assignment:titleAddIOTest")}</DialogTitle>
                <DialogContent style={{ overflow: "hidden" }}>
                    <Grid container spacing={2} direction="column">
                        <Grid item>
                            <Typography>
                                {t("assignment:bodyAddIOTest")}
                            </Typography>
                        </Grid>
                        <Grid item>
                            <TextFieldController
                                fullWidth
                                label={t("assignment:ioTestName")}
                                name="name"
                                control={control}
                                required
                                controllerProps={{
                        ...
                        <Grid item>
                            <MarkdownFieldController
                                fullWidth
                                label={t("assignment:ioTestDescription")}
                                name="description"
                                control={control}
                            />
                        </Grid>
                        ...
                        <Grid item data-cy={`addIOTestDialog_code`}>
                ...
                <DialogActions>
                    ...
                    <WaitingOverlay waiting={waiting}>
                        <Button
                            color="primary"
                            type="submit"
                            data-cy={`addIOTestDialog_submit`}
